...
class PredictionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        O, _ = torchaudio.load("./nt1_middle.wav", frame_offset=idx*X, num_frames=X)
        return I, O
...
if __name__ == "__main__":
    ...
    model = NVSR.load_from_checkpoint('/Users/mikesol/Downloads/model266.ckpt', map_location=torch.device('cpu'), channels=1, vocoder = True)
    # vocoder not used when doing transfer learning
    ...
    torchaudio.save(f"./temp/prediction-mic-transfer-learning-no-pp.wav", torch.cat([torch.from_numpy(x).unsqueeze(0) for x in prediction],1), 44100)