...
class PredictionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        I, _ = torchaudio.load("./67_near.wav", frame_offset=idx*X, num_frames=X)
        return I
...
if __name__ == "__main__":
    ...
    model = NVSR.load_from_checkpoint('/Users/mikesol/Downloads/model-272-compressor-nvsr.ckpt', map_location=torch.device('cpu'), channels=1)
    ...
    torchaudio.save(f"./temp/prediction-compressor.wav", torch.cat([torch.from_numpy(x).unsqueeze(0) for x in prediction],1), 44100)